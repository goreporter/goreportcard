...
func fileURL(dir, filename string) string {
	var fileURL string
	base := strings.TrimPrefix(dir, "repos"+string(filepath.Separator)+"src"+string(filepath.Separator))
	switch {
...
func GoTool(dir string, filenames, command []string) (float64, []FileSummary, error) {
	...
	if strings.Contains(command[len(command)-1], "misspell") && len(filenames) > 1000 {
		glog.Infoln("disabling misspell on large repo...")
...
outer:
	for out.Scan() {
		...
		filename = strings.TrimPrefix(filename, "repos"+string(filepath.Separator)+"src")
		for _, skip := range skipSuffixes {
		...
		if autoGenerated("repos" + string(filepath.Separator) + "src" + filename) {
			continue outer
		}
		...
		if fs.Filename == "" {
			...
			if strings.HasPrefix(filename, string(filepath.Separator)+"github.com") {
				sp := strings.Split(filename, string(filepath.Separator))
				if len(sp) > 3 {
					fs.Filename = strings.Join(sp[3:], string(filepath.Separator))
				}
	...
	// glog.Infoln("END: ", command, time.Now().Sub(started))
